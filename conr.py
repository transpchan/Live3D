...
class CoNR():
    def __init__(self, args):
        ...
        self.traget_pose_encoder = ResEncUnet(
    ...
    def dist(self):
        ...
        if args.distributed:
            self.udpparsernet = torch.nn.parallel.DistributedDataParallel(
                ...
                broadcast_buffers=args.broadcast_buffers_resnet,
                find_unused_parameters=True
            )
            self.traget_pose_encoder = torch.nn.parallel.DistributedDataParallel(
                self.traget_pose_encoder,
                device_ids=[
                    args.local_rank],
                output_device=args.local_rank,
                broadcast_buffers=args.broadcast_buffers_resnet,
            ...
            self.shader = torch.nn.parallel.DistributedDataParallel(
                ...
                device_ids=[
                    args.local_rank],
                output_device=args.local_rank,
                broadcast_buffers=args.broadcast_buffers_shader
            )
    ...
    def load_model(self, path):
        ...
        self.traget_pose_encoder.load_state_dict(
            torch.load('{}/traget_pose_encoder.pth'.format(path), map_location=device))
    ...
    def save_model(self, ite_num):
        ...
        self._save_pth(self.traget_pose_encoder,
                       model_name="traget_pose_encoder", ite_num=ite_num)
    ...
    def _save_pth(self,  net, model_name, ite_num):
        ...
        if to_save:
            model_dir = os.path.join(
                os.getcwd(), 'saved_models', args.model_name + args.runid + os.sep + "checkpoints" + os.sep + "itr_%d" % (ite_num)+os.sep)
            ...
            for name in ["character_labels", "pose_label"]:
                if name in data:
                    data[name] = data[name].to(
                        device, non_blocking=self.args.non_blocking).float()
            for name in ["pose_images", "pose_mask", "character_images", "character_masks"]:
                if name in data:
                    data[name] = data[name].to(
                        device, non_blocking=self.args.non_blocking).float() / 255.0
    ...
    def shader_forward(self, data, pred={}):
        if not("num_character_images" in data):
            print("WARN: Skip shader_forward. Pipeline may fail.")
            return pred
        ...
        if torch.any(torch.mean(shader_character_a_nmchw, (0, 2, 3, 4)) > 0.95):
            print(
                "WARN: no transparent area found in the image, use parser mask instead.")
            shader_character_a_nmchw = None
        if self.args.shader_character_use_parser_mask_test:
            shader_character_a_nmchw = None
    ...
    def pose_parser_sc_forward(self, data, pred={}):
        ...
        pred["pose_parser"]["sc_preds"] = UDPClip(encoder_out)
    ...
    def shader_pose_encoder_forward(self, data, pred={}):
        ...
        if "pose_label" in data:
            if torch.mean(data["pose_label"][:, 3:4, :, :]) > 0.001:
                shader_target_sudp = data["pose_label"][:, :3, :, :]

        if self.args.shader_pose_use_parser_udp_test:
            shader_target_sudp = None
        if shader_target_sudp is None:
            shader_target_sudp = pred["pose_parser"]["pred"][:, 0:3, :, :]

        if self.args.shader_pose_use_parser_mask_test:
            shader_target_a = None
